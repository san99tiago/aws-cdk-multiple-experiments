...
def _send_response(
    response_url: AnyStr, response_body: AnyStr, ssl_verify: Union[bool, AnyStr] = None
):
    try:
    ...
    if isinstance(ssl_verify, str):
        if path.exists(ssl_verify):
            ...
            logger.warning(
                "Cert path {0} does not exist!.  Falling back to using system cafile.".format(
                    ssl_verify
                )
            )
    ...
    while True:
        try:
            ...
            connection.request(
                method="PUT", url=url, body=json_response_body, headers=headers
            )
            response = connection.getresponse()
            logger.info(
                "CloudFormation returned status code: {}".format(response.reason)
            )
            break
        except Exception as e:
            logger.error(
                "Unexpected failure sending response to CloudFormation {}".format(e),
                exc_info=True,
            )
            time.sleep(5)
